    ...
    try {
      if (!lavalinkService.isConnected()) {
        await interaction.reply({
          content: '❌ Lavalink node не подключена. Попробуйте позже.',
      ...
      const searchResult = await lavalinkService.lavashark
        .search(searchQuery)
        .catch(() => null);
      ...
      if (searchResult.playlistInfo && searchResult.tracks.length > 1) {
        ...
        message = wasEmpty ? `▶️ Воспроизвожу: **${track.title}**` : `▶️ Добавлено в очередь: **${track.title}**`;
      }
      ...
      if (!interaction.replied) {
        await interaction.reply({
          content: '❌ Произошла ошибка при попытке воспроизвести трек.',
          flags: MessageFlags.Ephemeral,
        });
      }
    }
